...
class RecoveryPasswordAction extends CAction
{
    ...
    public function run($code)
    {
        ...
        {
            Yii::log(Yii::t(
                'user', 'Код восстановления пароля {code} не найден!', array('{code}' => $code)
            ), CLogger::LEVEL_ERROR, UserModule::$logCategory);
            Yii::app()->user->setFlash(YFlashMessages::ERROR_MESSAGE, Yii::t(
                'user', 'Код восстановления пароля не найден! Попробуйте еще раз!'
            ));
        ...
        if ($module->autoRecoveryPassword)
        {
                ...
                if ($recovery->user->save())
                {
                    if (RecoveryPassword::model()->deleteAll('user_id = :user_id', array(':user_id' => $recovery->user->id)))
                    {
                        ...
                        Yii::app()->user->setFlash(YFlashMessages::NOTICE_MESSAGE, Yii::t(
                            'user', 'Новый пароль отправлен Вам на email!'
                        ));
                        Yii::log(Yii::t(
                            'user', 'Успешное восстановление пароля!'
                        ), CLogger::LEVEL_ERROR, UserModule::$logCategory);
            ...
            catch (CDbException $e)
            {
                ...
                Yii::app()->user->setFlash(YFlashMessages::ERROR_MESSAGE, Yii::t(
                    'user', 'Ошибка при смене пароля!'
                ));
                Yii::log(Yii::t(
                    'user', 'Ошибка при автоматической смене пароля {error}!', array('{error}' => $e->getMessage())
                ), CLogger::LEVEL_ERROR, UserModule::$logCategory);
        ...
        if (Yii::app()->request->isPostRequest && !empty($_POST['ChangePasswordForm']))
        {
            ...
            if ($changePasswordForm->validate())
            {
                ...
                try
                {
                    ...
                    if ($recovery->user->save())
                    {
                        if(RecoveryPassword::model()->deleteAll('user_id = :user_id', array(':user_id' => $recovery->user->id)))
                        {
                            ...
                            Yii::log(Yii::t(
                                'user', 'Успешная смена пароля для пользоателя {user}!', array('{user}' => $recovery->user->id)
                ...
                catch (CDbException $e)
                {
                    ...
                    Yii::log(Yii::t(
                        'Ошибка при смене пароля {error}!', array('{error}' => $e->getMessage())
                    ), CLogger::LEVEL_ERROR, UserModule::$logCategory);
