...
class ProductVariant extends \yupe\models\YModel
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'position' => Yii::t('StoreModule.product', 'Порядок'),
        ];
    }
    ...
    public function getOptionValue($includeCost = false)
    {
        ...
        switch ($this->attribute->type) {
            case Attribute::TYPE_CHECKBOX:
                $value = $this->attribute_value ? Yii::t('StoreModule.product', 'Да') : Yii::t(
                    'StoreModule.product',
                    'Нет'
                );
                break;
        ...
        if ($includeCost) {
            switch ($this->type) {
                ...
                case self::TYPE_BASE_PRICE:
                    $value .= ' (' . Yii::t("StoreModule.product", 'цена') . ': ' . $this->amount . ' ' . Yii::t(
                            "StoreModule.product",
                            'руб'
                        ) . '.)';
                    break;
