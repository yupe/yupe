...
class Bootstrap extends CApplicationComponent
{
    ...
    {
        Yii::app()->getClientScript()->scriptMap['jquery-ui.css'] = $this->getAssetsUrl(
    ...
    public function registerJS($position = CClientScript::POS_HEAD)
    {
        ...
        $cs->registerScriptFile(
            $this->getAssetsUrl() . '/js/bootstrap' . (!YII_DEBUG ? '.min' : '') . '.js',
            $position
        );
    }
    ...
    public function registerDateRangePlugin($selector, $options = array(), $callback = null)
    {
        ...
        Yii::app()->clientScript->registerScript(
            $key,
            '$("' . $selector . '").daterangepicker(' . CJavaScript::encode(
                $options
