...
class LanguageBehavior extends CBehavior
{
    ...
    public function handleLanguageBehavior($event)
    {
        ...
        if ( (isset($_GET[$lm->langParam]) && in_array($_GET[$lm->langParam],$lm->languages) && ($l=$_GET[$lm->langParam]))
        || ( ($l = substr(Yii::app()->request->getPathInfo(),0,2)) && (2==strlen($l)) && in_array($l,$lm->languages)  )
        )
        {
