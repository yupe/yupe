...
class LanguageBehavior extends CBehavior
{
        ...
        $l = substr($path, 0, 2) &&
            ...
            if ($app->hasModule('user')) {
                ...
                if ($user->hasState($lm->langParam)) {
                    ...
                    if ($app->getModule('yupe')->cache && isset($app->request->cookies[$lm->langParam]) && in_array(
                        $app->request->cookies[$lm->langParam]->value,
                        $lm->languages
                    )
                    ) {
                        ...
                        if ($lm->preferredLanguage && $l = $app->request->getPreferredLanguage()) {
    ...
    protected function setLanguage($language)
    {
        ...
        if (Yii::app()->getModule('yupe')->cache) {
            Yii::app()->request->cookies[Yii::app()->urlManager->langParam] = new CHttpCookie(Yii::app(
            )->urlManager->langParam, $language, array('expire' => time() + (60 * 60 * 24 * 365)));
        }
