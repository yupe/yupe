...
<?php $subject = Yii::t('ShopModule.shop', 'Заказ №{n} в магазине {site}', array('{n}' => $order->id, '{site}' => Yii::app()->getModule('yupe')->siteName)); ?>
<h1 style="font-weight:normal;">
    <?php echo CHtml::link("Ваш заказ №{$order->id}", Yii::app()->createAbsoluteUrl('/shop/order/view', array('url' => $order->url))); ?>
    на сумму <?php echo Yii::t('ShopModule.shop', '{n} рубль|{n} рубля|{n} рублей', array($order->total_price)); ?>
    <?php if ($order->paid): ?>
        оплачен,
    <?php else: ?>
        еще не оплачен,
...
<?php echo CHtml::link(Yii::app()->createAbsoluteUrl('/shop/order/view', array('url' => $order->url)), Yii::app()->createAbsoluteUrl('/shop/order/view', array('url' => $order->url))); ?>
<br>
