...
class CatalogController extends yupe\components\controllers\FrontController
{
    ...
    public function actionShow($name)
    {
        ...
        if (!$products)
        {
            throw new CHttpException(404, Yii::t('ShopModule.catalog', 'Product was not found!'));
        }
    ...
    public function actionAutocomplete()
    {
        ...
        if (strlen($term) > 2)
        {
            ...
            //$qtxt    = "SELECT trim(CONCAT(COALESCE(`sku`,''), ' ' , name))  FROM {{shop_product}} WHERE name LIKE :search or sku LIKE :search";
            $qtxt    = "SELECT name  FROM {{shop_product}} WHERE name LIKE :search or sku LIKE :search";
