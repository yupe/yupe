 ...
 * Модуль engine - основной модуль системы.
 *
 * Модуль engine содержит в себе все основные компоненты, которые используются другими модулями
...
class YupeModule extends components\YWebModule
{
    ...
    public $coreModuleId = 'engine';
    public $editorsDir = 'application.modules.engine.widgets.editors';
    ...
    {
        return Yii::t('YupeModule.engine', '0.5.4');
    }
            ...
            $messages[YWebModule::CHECK_ERROR][] = array(
                ...
                'message' => Yii::t(
                    ...
                    array(
                        '{dir}' => $uploadPath,
                        '{link}' => CHtml::link(
                            Yii::t('YupeModule.engine', 'Изменить настройки'),
                            array('/engine/backend/modulesettings/', 'module' => 'engine')
                        ),
                    )
                ),
            );
    ...
    {
        return array(
            'siteDescription' => Yii::t('YupeModule.engine', 'Описание сайта'),
            'siteName' => Yii::t('YupeModule.engine', 'Название сайта'),
            'siteKeyWords' => Yii::t('YupeModule.engine', 'Ключевые слова сайта'),
            'backendLayout' => Yii::t('YupeModule.engine', 'Layout административной части'),
            'backendTheme' => Yii::t('YupeModule.engine', 'Тема административной части'),
            'theme' => Yii::t('YupeModule.engine', 'Тема сайта'),
            'coreCacheTime' => Yii::t('YupeModule.engine', 'Время кэширования (сек.)'),
            'editorsDir' => Yii::t('YupeModule.engine', 'Каталог для визивиг редакторов'),
            'uploadPath' => Yii::t('YupeModule.engine', 'Каталог для загрузки файлов (относительно корня сайта)'),
            'editor' => Yii::t('YupeModule.engine', 'Визуальный редактор'),
            'email' => Yii::t('YupeModule.engine', 'Email администратора'),
            'availableLanguages' => Yii::t('YupeModule.engine', 'Список доступных языков через запятую (напр. ru,en,de)'),
            'defaultLanguage' => Yii::t('YupeModule.engine', 'Язык по умолчанию для сайта'),
            'defaultBackendLanguage' => Yii::t('YupeModule.engine', 'Язык по умолчанию для панели управления'),
            'updateChannel' => Yii::t('YupeModule.engine', 'Обновление Yupe'),
        );
    }
    ...
    public function getEditableParamsGroups()
    {
        return array(
            'main' => array(
                'label' => Yii::t('YupeModule.engine', 'Основные настройки'),
            ),
            'theme' => array(
                'label' => Yii::t('YupeModule.engine', 'Настройка внешнего вида'),
                'items' => array(
                    'backendLayout',
                    'backendTheme',
                )
            ...
            'language' => array(
                'label' => Yii::t('YupeModule.engine', 'Языковые настройки'),
                'items' => array(
                    'availableLanguages',
                    'defaultLanguage',
            ...
            'editors' => array(
                'label' => Yii::t('YupeModule.engine', 'Настройки редакторов'),
                'items' => array(
                    'editorsDir',
                    'uploadPath',
                    'editor',
    ...
    {
        $channelsList = array(
            'disabled' => Yii::t('YupeModule.engine', 'Обновление отключено'),
            'release' => Yii::t('YupeModule.engine', 'Обновление релизами'),
        );
        return $channelsList;
    }
    ...
    {
        return array('/engine/backend/modulesettings', 'module' => 'engine');
    }
    ...
    public function getNavigation()
    {
        return array(
            array(
                'label' => Yii::t('YupeModule.engine', 'Очистить кеш'),
                'url' => 'javascript::void();',
                'icon' => 'trash',
                'items' => array(
                    array(
                        'icon' => 'trash',
                        'label' => Yii::t('YupeModule.engine', 'Сбросить кеш настроек'),
                        'url' => array('/engine/backend/flushDumpSettings'),
                        'linkOptions' => array(
                            'class' => 'flushAction',
                            'method' => 'cacheFlush',
                        )
                    ...
                    array(
                        'icon' => 'trash',
                        'label' => Yii::t('YupeModule.engine', 'Очистить кеш'),
                        'url' => array('/engine/backend/ajaxflush', 'method' => 1),
                        'linkOptions' => array(
                            'class' => 'flushAction',
                            'method' => 'cacheFlush',
                        )
                    ...
                    array(
                        'icon' => 'trash',
                        'label' => Yii::t('YupeModule.engine', 'Очистить ресурсы (assets)'),
                        'url' => array('/engine/backend/ajaxflush', 'method' => 2),
                        'linkOptions' => array(
                            'class' => 'flushAction',
                            'method' => 'assetsFlush',
                        )
                    ...
                    array(
                        'icon' => 'trash',
                        'label' => Yii::t('YupeModule.engine', 'Очистить кеш и ресурсы (assets)'),
                        'url' => array('/engine/backend/ajaxflush', 'method' => 3),
                        'linkOptions' => array(
                            'class' => 'flushAction',
            ...
            array(
                'icon' => 'picture',
                'label' => Yii::t('YupeModule.engine', 'Оформление'),
                'url' => array('/engine/backend/themesettings'),
            ),
            array(
                'icon' => 'wrench',
                'label' => Yii::t('YupeModule.engine', 'Параметры сайта'),
                'url' => array(
                    '/engine/backend/modulesettings',
                    'module' => 'engine',
                ),
            ),
        );
    ...
    {
        return Yii::t('YupeModule.engine', 'Юпи!');
    }
    ...
    {
        return Yii::t('YupeModule.engine', 'Ядро Юпи!');
    }
    ...
    {
        return Yii::t('YupeModule.engine', 'engine team');
    }
    ...
    {
        return Yii::t('YupeModule.engine', 'team@engine.ru');
    }
    ...
    {
        return Yii::t('YupeModule.engine', 'http://engine.ru');
    }
    ...
    public function init()
    {
        ...
        $this->otherCategoryName = Yii::t('YupeModule.engine', 'Остальное');
        ...
        $this->categoryIcon = array(
            Yii::t('YupeModule.engine', 'Сервисы') => 'briefcase',
            $this->otherCategoryName => 'cog',
        );

        $this->categorySort = array(
            Yii::t('YupeModule.engine', 'Контент'),
            Yii::t('YupeModule.engine', 'Структура'),
            Yii::t('YupeModule.engine', 'Пользователи'),
            Yii::t('YupeModule.engine', 'Сервисы'),
            Yii::t('YupeModule.engine', 'Юпи!'),
    ...
    public function getModules($navigationOnly = false, $disableModule = false)
    {
        ...
        if (count(Yii::app()->modules)) {
            ...
            if ($modulesNavigation === false) {
                ...
                $settings = array(
                    'icon' => "wrench",
                    'label' => Yii::t('YupeModule.engine', 'Модули'),
                    'url' => array('/engine/backend/settings'),
                    'items' => array(),
                );
                    ...
                    foreach ($valueCategory as $key => $value) {
                        ...
                        if ($modules[$key]->editableParams && $key != $this->id) {
                            $settings['items'][] = array(
                                ...
                                'url' => array('/engine/backend/modulesettings', 'module' => $modules[$key]->id),
                            );
                        }
                ...
                $chain->dependencies->add(
                    new TagsCache('engine', 'navigation', 'installedModules')
                );
        ...
        if (CHtml::normalizeUrl("/" . Yii::app()->controller->route) != '/engine/backend/index'
            ...
            $thisModule = (
                (Yii::app(
                )->controller->action->id == 'modulesettings' && isset($_GET['module']) && $_GET['module'] != 'engine') ||
    ...
    public function getModulesDisabled($enableModule = array())
    {
        ...
        if ($imports === false || ($modules = Yii::app()->cache->get('modulesDisabled')) === false) {
            ...
            // Зависимость на тег:
            $chain->dependencies->add(
                new TagsCache('engine', 'modulesDisabled', 'getModulesDisabled', 'installedModules', 'pathForImports')
            );
    ...
    public function getBackendLayoutAlias($layoutName = '')
    {
        if ($this->backendTheme) {
            return 'webroot.themes.backend_' . $this->backendTheme . '.views.engine.layouts.' . ($layoutName ? $layoutName : $this->backendLayout);
        } else {
            return 'application.modules.engine.views.layouts.' . ($layoutName ? $layoutName : $this->backendLayout);
        }
    }
     ...
     * Скопировать каталог с виджетом редактора в application.modules.engine.widgets.editors
    ...
    public function getLanguageSelectorArray()
    {
        ...
        foreach ($langs as $lang) {
            if ($lang == $currentLanguage) {
                ...
                $items[] = array(
                    ...
                    'label' => Yii::t('YupeModule.engine', $lang),
        ...
        return array(
            array(
                ...
                'label' => Yii::t('YupeModule.engine', $currentLanguage),
                'items' => $items,
    ...
    {
        if (empty($text)) {
            $text = Yii::t('YupeModule.engine', 'Работает на Юпи!');
        }
        return CHtml::link(
            CHtml::image(Yii::app()->baseUrl . "/web/images/yupe_{$color}.png", $text),
            'http://engine.ru?from=pb',
