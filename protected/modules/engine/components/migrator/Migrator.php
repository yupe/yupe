 ...
 * @license  BSD https://raw.github.com/engine/engine/master/LICENSE
 * @version  0.5.3
 * @link     http://www.engine.ru
 */
...
class Migrator extends CApplicationComponent
{
    ...
    {
        echo Yii::t('YupeModule.engine', "Проверяем на наличие незавершённых миграций.") . '<br />';
    ...
    protected function migrateUp($module, $class)
    {
        ...
        echo Yii::t('YupeModule.engine', "Применяем миграцию {class}", array('{class}' => $class));
    ...
    {
        Yii::log(Yii::t('YupeModule.engine', "Отменяем миграцию {class}", array('{class}' => $class)));
        $db = $this->getDbConnection();
        $start = microtime(true);
        $migration = $this->instantiateMigration($module, $class);
    ...
    protected function getDbConnection()
    {
        ...
        throw new CException(
            Yii::t('YupeModule.engine', 'Неверно указан параметр connectionID')
        );
    }
    ...
    public function getMigrationHistory($module, $limit = 20)
    {
        ...
        if ($allData === false || !isset($allData[$module])) {
            ...
            Yii::app()->cache->set('getMigrationHistory', $allData, 3600, new TagsCache('engine', 'installedModules', 'getModulesDisabled', 'getMigrationHistory', $module));
