...
class YFeedAction extends CAction
{
     ...
     *         return array(
     *             'atomfeed' => array(
     *                 'class'        => 'application.modules.engine.components.actions.YFeedAction',
     *                 'data'         => News::model()->published()->findAll(),
     *                 // Параметр title по умолчанию берётся из настроек приложения
     *                 //'title'        => Yii::t('YupeModule.engine', 'Название моего сайта'),
     *                 // Параметр description по умолчанию берётся из настроек приложения
     *                 //'description'  => Yii::t('YupeModule.engine', 'Лента новостей сайта'),
    ...
    public function run()
    {
        ...
        Yii::import('application.modules.engine.extensions.feed.*');
        ...
        if (count($this->data) > 0) {
             ...
             */
            if (!empty($this->itemFields['content']))
                $item->description = Yii::t('YupeModule.engine', 'Записей нет');
