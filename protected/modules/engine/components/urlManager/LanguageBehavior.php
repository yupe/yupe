 ...
 * @link     http://engine.ru
 */
namespace yupe\modules\yupe\components\urlManager;
...
class LanguageBehavior extends \CBehavior
{
    ...
    public function handleLanguageBehavior($event)
    {
        ...
        if (in_array($this->lang, $lm->languages) && $langIsset) {
            ...
            $cookiesLang = Yii::app()->getModule('engine')->cache
            ...
            if ($oldLang != $this->lang
                ...
                Yii::app()->user->setFlash(
                    ...
                    Yii::t(
                        'YupeModule.engine', 'Язык "{lang}" не известен системе!', array(
                            '{lang}' => $oldLang
                        )
                    )
                );
    ...
    protected function setLanguage($language)
    {
        ...
        if (Yii::app()->getModule('engine')->cache) {
