...
abstract class YWebModule extends CWebModule
{
     ...
     * @var array редактор
     */
    public $editor = 'application.modules.engine.widgets.editors.imperaviRedactor.ImperaviRedactorWidget';
    /**
    ...
    {
        return Yii::t('YupeModule.engine', 'Сообщество Юпи!');
    }
     ...
     *                '{link}' => CHtml::link(Yii::t('image', 'Изменить настройки модуля'), array('/engine/backend/modulesettings/', 'module' => $this->id))
     *            ))
     *        );
    ...
    {
        return array('adminMenuOrder' => Yii::t('YupeModule.engine', 'Порядок следования в меню'));
    }
    ...
    public function getEditableParamsGroups()
    {
        return array(
            'main' => array(
                'label' => Yii::t('YupeModule.engine', 'Основные настройки модуля'),
                'items' => array(
                    'adminMenuOrder'
                )
            ),
    ...
    public function getModulesNoDisable()
    {
        ...
        if ($modulesNoDisable === false) {
            $modules = Yii::app()->getModule('engine')->getModules(false, true);
            ...
            Yii::app()->cache->set(
                ...
                $modulesNoDisable,
                Yii::app()->getModule('engine')->coreCacheTime,
                new TagsCache('engine', 'installedModules')
            );
        }
    ...
    public function getDependenciesAll()
    {
        ...
        if ($modulesDependent === false) {
            ...
            Yii::app()->cache->set(
                ...
                $modulesDependent,
                0,
                new TagsCache('installedModules', 'engine')
            );
        }
    ...
    public function getIsInstalled()
    {
        ...
        if ($modulesInstalled === false) {
            ...
            // Зависимость на тег:
            $chain->dependencies->add(
                new TagsCache('installedModules', 'disabledModules', 'engine', $this->getId())
            );
    ...
    public function getActivate($noDependen = false, $updateConfig = false)
    {
        ...
        if (is_file($fileConfig) && $this->id != 'install' && $updateConfig === false) {
            throw new CException(Yii::t('YupeModule.engine', 'Модуль уже включен!'), 304);
        } else {
    ...
    public function getDeActivate($noDependen = false)
    {
        ...
        if (!is_file($fileConfig) && $this->id != 'install') {
            throw new CException(Yii::t('YupeModule.engine', 'Модуль уже отключен!'));
        } else {
            ...
            if ($this->getIsNoDisable()) {
                throw new CException(Yii::t('YupeModule.engine', 'Этот модуль запрещено отключать!'));
    ...
    {
        return ($this->id == 'engine' || $status = $this->getActivate()) ? $this->installDB() : $status;
    }
    ...
    {
        if ($this->getIsActive()) {
            throw new CException(Yii::t('YupeModule.engine', 'Сначала отключите модуль!'));
        }
    ...
    public function uninstallDB()
    {
        ...
        if (!empty($history)) {
            ...
            Yii::app()->cache->clear('installedModules', $this->getId(), 'engine', 'getModulesDisabled', 'modulesDisabled', $this->getId());
            application\modules\yupe\components\ConfigManager::flushDump();
        ...
        throw new CException(Yii::t('YupeModule.engine', 'Произошла ошибка удаления БД модуля!'));
    }
    ...
    {
        return array(
            self::CHOICE_YES => Yii::t('YupeModule.engine', 'да'),
            self::CHOICE_NO  => Yii::t('YupeModule.engine', 'нет'),
        );
    }
    ...
    {
        $uploadController = Yii::app()->createUrl('/engine/backend/AjaxFileUpload');
