...
class YBackController extends YMainController
{
    ...
    {
        return array(
            array('application.modules.engine.filters.YBackAccessControl'),
        );
    }
    ...
    {
        parent::init();
        //$this->engine->getComponent('bootstrap');
        $this->layout = $this->yupe->getBackendLayoutAlias();
        $backendTheme = $this->yupe->backendTheme;
        $this->setPageTitle(Yii::t('YupeModule.engine', 'Панель управления Юпи!'));
    ...
    protected function beforeAction($action)
    {
        ...
        if ($this->module->getId() !== 'install'
            ...
            Yii::app()->user->setFlash(
                ...
                Yii::t('YupeModule.engine', 'Перед тем как начать работать с модулем, необходимо установить все необходимые миграции.')
            );

            $this->redirect(array('/engine/backend/modupdate', 'name' => $this->module->getId()));
        }
    ...
    public function actionMultiaction()
    {
        ...
        try {
            ...
            case self::BULK_DELETE:
                ...
                Yii::app()->ajax->success(
                    Yii::t(
                        'YupeModule.engine', 'Удалено {count} записей!', array(
                            '{count}' => $count
                        )
                    )
                );
