    ...
    */
    public function getUpDownButtons($data)
    {
        $downUrlImage = CHtml::image(
            ...
            Yii::t('YupeModule.engine', 'Опустить ниже'),
            array('title' => Yii::t('YupeModule.engine', 'Опустить ниже'))
        );

        $upUrlImage = CHtml::image(
            Yii::app()->assetManager->publish(Yii::getPathOfAlias('zii.widgets.assets.gridview') . '/up.gif'),
            Yii::t('YupeModule.engine', 'Поднять выше'),
            array('title' => Yii::t('YupeModule.engine', 'Поднять выше'))
        );
    ...
    public function renderHeadline()
    {
        ...
        /* Текстовка: */
        echo Yii::t('YupeModule.engine', 'Отображать по:') . '<br />';
        ...
        $cscript->registerScript(
            ...
            'var multiaction = function(action, values) {
                ...
                $.ajax({
                    ...
                    error: function(data){alert("'.Yii::t('YupeModule.engine','Произошла ошибка!').'")}
                });
