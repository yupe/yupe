 ...
 * @package  YupeCms
 * @author   YupeTeam <team@engine.ru>
...
class BackendController extends YBackController
{
    ...
    public function actionFlushDumpSettings()
    {
        if (Yii::app()->request->isAjaxRequest == false) {
            throw new CHttpException(404, Yii::t('YupeModule.engine', 'Страница не существует!'));
        }

        $json = array();
        $message = array(
            'success' => Yii::t('YupeModule.engine', 'Сброс кеш-файла настроек успешно завершён'),
            'failure' => Yii::t('YupeModule.engine', 'Во время обработки запроса произошла ошибка'),
        );
        ...
        try {
            ...
            Yii::app()->ajax->failure(
                Yii::t(
                    'YupeModule.engine', 'Произошла ошибка: {error}', array(
    ...
    public function actionModulesettings($module)
    {
        if (!($module = Yii::app()->getModule($module)))
            throw new CHttpException(404, Yii::t('YupeModule.engine', 'Страница настроек данного модуля недоступна!'));
                ...
                $element = CHtml::label($moduleParamsLabels[$key], $key)
                        . CHtml::dropDownList(
                            $key, $value, $editableParams[$key], array(
                                'empty' => Yii::t('YupeModule.engine', '--выберите--') ,
                                'class' => 'span10'
                            )
                        );
            } else {
    ...
    public function actionSaveModulesettings()
    {
        if (Yii::app()->request->isPostRequest) {
            if (!($moduleId = Yii::app()->request->getPost('module_id')))
                throw new CHttpException(404, Yii::t('YupeModule.engine', 'Страница не найдена!'));

            if (!($module = Yii::app()->getModule($moduleId)))
                throw new CHttpException(404, Yii::t('YupeModule.engine', 'Модуль "{module}" не найден!', array('{module}' => $moduleId)));

            if (!$this->saveParamsSetting($moduleId, $module->editableParamsKey)) {
                Yii::app()->user->setFlash(
                    YFlashMessages::SUCCESS_MESSAGE,
                    Yii::t(
                        'YupeModule.engine', 'Настройки модуля "{module}" сохранены!', array(
                ...
                Yii::app()->user->setFlash(
                    ...
                    Yii::t('YupeModule.engine', 'При сохранении произошла ошибка!')
                );
            }
            $this->redirect(array('/engine/backend/modulesettings', 'module' => $moduleId));
        }
        throw new CHttpException(404, Yii::t('YupeModule.engine', 'Страница не найдена!'));
    }
    ...
    public function actionThemesettings()
    {
        if (Yii::app()->request->isPostRequest) {
            if (!$this->saveParamsSetting($this->yupe->coreModuleId, array('theme', 'backendTheme'))) {
                Yii::app()->user->setFlash(
                    YFlashMessages::SUCCESS_MESSAGE,
                    Yii::t('YupeModule.engine', 'Настройки тем сохранены!')
                );
                Yii::app()->cache->clear('engine');
            }
            else
                Yii::app()->user->setFlash(
                    YFlashMessages::ERROR_MESSAGE,
                    Yii::t('YupeModule.engine', 'При сохранении настроек произошла ошибка!')
                );
            $this->redirect(array('/engine/backend/themesettings/'));
        }

        $settings = Settings::model()->fetchModuleSettings('engine', array('theme', 'backendTheme'));
        $noThemeValue = Yii::t('YupeModule.engine', 'Тема не используется');
        ...
        if ($name) {
            ...
            if ($module->getIsInstalled()) {
                ...
                if (Yii::app()->request->isPostRequest) {
                    ...
                    Yii::app()->user->setFlash(
                        YFlashMessages::SUCCESS_MESSAGE,
                        Yii::t('YupeModule.engine', 'Модуль обновил свои миграции!')
                    );
                    $this->redirect(array("/engine/backend"));
                } else
                    $this->render('modupdate', array('updates' => $updates, 'module' => $module));
            } else
                Yii::app()->user->setFlash(
                    YFlashMessages::ERROR_MESSAGE,
                    Yii::t('YupeModule.engine', 'Модуль еще не установлен!')
                );
        } else {
            Yii::app()->user->setFlash(
                YFlashMessages::ERROR_MESSAGE,
                Yii::t('YupeModule.engine', 'Не указано имя модуля!')
            );

            $this->redirect(Yii::app()->request->urlReferrer !== null ? Yii::app()->request->urlReferrer : array("/engine/backend"));
        }
    }
    ...
    public function actionAjaxFileUpload()
    {
        if (!empty($_FILES['file']['name'])) {
            ...
            if (!is_dir($uploadPath)) {
                if (!@mkdir($uploadPath))
                    Yii::app()->ajax->rawText(Yii::t('YupeModule.engine', 'Не удалось создать каталог "{dir}" для файлов!', array('{dir}' => $uploadPath)));
            }
            ...
            if ($file) {
                ...
                if (!$file->saveAs($uploadPath . $newFileName))
                    Yii::app()->ajax->rawText(Yii::t('YupeModule.engine', 'При загрузке произошла ошибка!'));
    ...
    public function actionModulestatus()
    {
        ...
        elseif (!isset($status) || !in_array($status, array(0, 1)))
            Yii::app()->ajax->failure(Yii::t('YupeModule.engine', 'Не указан статус для обработки!'));
            ...
            try {
                ...
                case 0:
                    if ($module->getIsActive()) {
                        ...
                        $message = Yii::t('YupeModule.engine', 'Модуль успешно отключен!');
                    } else {
                        $module->getUnInstall();
                        $message = Yii::t('YupeModule.engine', 'Модуль успешно деинсталлирован!');
                    }
                    $result = true;
                    break;
                ...
                case 1:
                    if ($module->getIsInstalled()) {
                        $module->getActivate();
                        $message = Yii::t('YupeModule.engine', 'Модуль успешно включен!');
                    } else {
                        $module->getInstall();
                        $message = Yii::t('YupeModule.engine', 'Модуль успешно установлен!');
                    }
                    $result = true;
                    break;
                case 2:
                    $message = ($result = $module->getActivate(false, true)) === true
                        ? Yii::t('YupeModule.engine', 'Файл настроек модуля "{n}" успешно обновлён!', $name)
                        : Yii::t('YupeModule.engine', 'При обновлении файла настроек модуля "{n}" произошла ошибка!', $name);
                    Yii::app()->user->setFlash(
                    ...
                    break;
                default:
                    $message = Yii::t('YupeModule.engine', 'Выбрано неизвестное действие!');
                    break;
                }
         ...
         **/
            Yii::app()->ajax->failure(Yii::t('YupeModule.engine', 'Модуль не найден или его включение запрещено!'));
    }
    ...
    public function actionAjaxflush()
    {
        ...
        case 'cacheFlush':
            try {
                ...
                Yii::app()->ajax->success(
                    Yii::t('YupeModule.engine', 'Очистка кеша успешно завершена!')
                );
            } catch(Exception $e) {
                Yii::app()->ajax->failure(
                    $e->getMessage()
        ...
        case 'assetsFlush':
            if ($this->_cleanAssets()) {
                Yii::app()->ajax->success(
                    Yii::t('YupeModule.engine', 'Очистка ресурсов успешно завершена!')
                );
            }
            break;
        /**
        ...
        case 'cacheAssetsFlush':
            try {
                ...
                if ($this->_cleanAssets()) {
                    Yii::app()->ajax->success(
                        Yii::t('YupeModule.engine', 'Очистка ресурсов и кеша успешно завершена!')
                    );
                }
            } catch(Exception $e) {
                Yii::app()->ajax->failure(
         ...
         **/
        default:
            Yii::app()->ajax->failure(Yii::t('YupeModule.engine', 'Использован неизвестный системе метод!'));
            break;
        }
    }
        ...
        if (Yii::app()->request->isPostRequest && ($bugData = Yii::app()->request->getPost('BugForm'))) {
            ...
            if ($form->validate()) {
                if ($form->module === BugForm::OTHER_MODULE){
                    $form->module = Yii::t('YupeModule.engine', 'Другой модуль');
                }
                Yii::app()->mail->send(
                    Yii::app()->user->email,
                    $form->sendTo,
                ...
                Yii::app()->user->setFlash(
                    YFlashMessages::SUCCESS_MESSAGE,
                    Yii::t('YupeModule.engine', 'Сообщение отправлено!')
                );
                $this->redirect('/engine/backend/reportBug');
            }
        }
